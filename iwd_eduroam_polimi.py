...
def generate_iwd_configuration(certfile_destination, keyfile_destination, username):
    return IWD_FILE_TEMPLATE.format(certfile_destination=certfile_destination, keyfile_destination=keyfile_destination, username=username)


def generate_bag_attribute(subject, issuer):
    return BAG_ATTRIBUTE_TEMPLATE.format(friendlyName=subject, subject=', '.join(subject.replace('=', ' = ').split(',')[::-1]), issuer=', '.join(issuer.replace('=', ' = ').split(',')[::-1]))


def install(p12_certificate_path: str, username: str, password: str, install_system_wide: bool) -> None:
    private_key = None
    certificate = None
    additional_certificates = None
    try:
        with open(p12_certificate_path, "rb") as certificate_file:
            private_key, certificate, additional_certificates = pk.load_key_and_certificates(
                certificate_file.read(), password.encode())
    ...
    if install_system_wide:
        if os.path.exists(os.path.join(SYSTEM_CERTS_DIRECTORY, KEY_FILENAME)):
            print(
                f"Key {KEY_FILENAME} already exists in {SYSTEM_CERTS_DIRECTORY}, files will only be created in current directory")
        elif os.path.exists(os.path.join(SYSTEM_CERTS_DIRECTORY, CERTIFICATE_FILENAME)):
            print(
                f"Certificate {CERTIFICATE_FILENAME} already exists in {SYSTEM_CERTS_DIRECTORY}, files will only be created in current directory")
        elif os.path.exists(os.path.join(IWD_CONFIGURATION_DIRECTORY, IWD_EDUROAM_CONFIGURATION_FILENAME)):
            print(
                f"Configuration file {IWD_EDUROAM_CONFIGURATION_FILENAME} already exists in {IWD_CONFIGURATION_DIRECTORY}, files will only be created in current directory")
        elif os.path.exists(os.path.join(IWD_CONFIGURATION_DIRECTORY, IWD_POLIMIPROTECTED_CONFIGURATION_FILENAME)):
            print(
                f"Configuration file {IWD_POLIMIPROTECTED_CONFIGURATION_FILENAME} already exists in {IWD_CONFIGURATION_DIRECTORY}, files will only be created in current directory")
        else:
            ...
            print(
                f"To install them, manually place {KEY_FILENAME} and {CERTIFICATE_FILENAME} in {SYSTEM_CERTS_DIRECTORY}, and {IWD_EDUROAM_CONFIGURATION_FILENAME} and {IWD_POLIMIPROTECTED_CONFIGURATION_FILENAME} in {IWD_CONFIGURATION_DIRECTORY}")
            print("")

    keyfile_destination = os.path.join(certs_destination, KEY_FILENAME)
    with open(keyfile_destination, "wb") as keyfile_output:
        keyfile_output.write(private_key.private_bytes(
            encoding=ser.Encoding.PEM, format=ser.PrivateFormat.PKCS8, encryption_algorithm=ser.NoEncryption()))
    ...
    with open(certfile_destination, "wb") as certfile_output:
        ...
        for additional_certificate in additional_certificates:
            certfile_output.write(generate_bag_attribute(
                additional_certificate.subject.rfc4514_string(), additional_certificate.issuer.rfc4514_string()).encode())
...
def prepare_installer() -> Tuple[str, str, str, str]:
    ...
    parser = argparse.ArgumentParser(
        description='Configure iwd for eduroam and polimi-protected with a certificate.')
    parser.add_argument('--username', '-u', action='store',
                        dest='username', help='polimi person code', required=True)
    parser.add_argument('--certificate', '-c', action='store',
                        dest='certificate_path', help='certificate .p12 file', required=True)
    parser.add_argument('--install', '-i', default=False, action='store_true', dest='install',
                        help='install the generated files instead of just creating them in the local directory')
    args = parser.parse_args()

    if args.install and not os.geteuid() == 0:
        print("To install the generated configuration and certificates in your system, run this script with sudo")
        sys.exit(1)
